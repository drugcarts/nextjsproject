...
export async function PUT(request, { params }) {
    try {
        ...
        const existingTestPackage = await TestPackage.findById(id);
        if (!existingTestPackage) {
            return NextResponse.json({ error: 'TestPackage not found' }, { status: 404 });
        }

        let newImageKey = existingTestPackage.image;
        ...
        const updatedTestPackage = await TestPackage.findByIdAndUpdate(id, updatedData, { new: true });
